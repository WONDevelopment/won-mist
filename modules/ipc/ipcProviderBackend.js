...
class IpcProviderBackend {
    ...
    switch (
      ...
      case ethereumNode.STATES.STOPPING:
        ...
        _.each(this._remoteSubscriptions, remoteSubscriptionId => {
          ethereumNodeRemote.send('won_unsubscribe', [remoteSubscriptionId]);
        });
        this._remoteSubscriptions = {};
        this._subscriptionOwners = {};
  ...
  _sanitizeRequestPayload(conn, payload) {
    if (_.isArray(payload)) {
      _.each(payload, p => {
        if (p.method === 'won_sendTransaction') {
  ...
  /**
    Handles won_subscribe|won_subscription|won_unsubscribe to:
  ...
  _handleSubscriptions(result) {
    if (result.method === 'won_subscribe') {
    ...
    if (result.method === 'won_subscription') {
  ...
  _subscribeRemote(localSubscriptionId, params, retry = false) {
    return new Promise(async (resolve, reject) => {
      ...
      const requestId = await ethereumNodeRemote.send('won_subscribe', params);
