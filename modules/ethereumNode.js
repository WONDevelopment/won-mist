...
class EthereumNode extends EventEmitter {
  ...
  get isEth() {
    return this._type === 'won';
  }

  get isGeth() {
    return this._type === 'gwon';
  }
  ...
  _start(nodeType, network, syncMode) {
    ...
    return this.stop()
      ...
      .catch(err => {
        ...
        if (nodeType === 'won') {
          Settings.saveUserData('node', 'gwon');
        }
        ...
        default:
          args =
            nodeType === 'gwon'
              ...
              : ['--unsafe-transactions'];
          if (nodeType === 'gwon' && syncMode === 'nosync') {
            args.push('--nodiscover', '--maxpeers=0');
          } else {
            args.push('--syncmode', syncMode);
          }
    ...
    if (STATES.STARTING === this.state) {
      ...
      if (nodeType === 'gwon') {
        if (dataStr.indexOf('fatal: error') >= 0) {
          const error = new Error(`Gwon error: ${dataStr}`);
  ...
  _checkSync() {
    ...
    this.syncInterval = setInterval(async () => {
      const syncingResult = await this.send('won_syncing');
      const sync = syncingResult.result;
      if (sync === false) {
        const blockNumberResult = await this.send('won_blockNumber');
  ...
  _watchLocalBlocks() {
    ...
    this.watchlocalBlocksInterval = setInterval(async () => {
      const blockResult = await this.send('won_getBlockByNumber', [
        'latest',
        false
      ]);
  ...
  async getNetwork() {
    const blockResult = await this.send('won_getBlockByNumber', ['0x0', false]);
    const block = blockResult.result;
    switch (block.hash) {
      case '0x7a027e2464b52d43bb8bc0145484cf866bdab41dfdd86f69bde335211ff80601':
        return 'main';
