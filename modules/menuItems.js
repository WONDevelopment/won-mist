...
let menuTempl = function(webviews) {
        ...
        submenu: [
          {
            ...
            click() {
              ...
              if (ethereumNode.isEth) {
                ...
                if (process.platform === 'darwin') {
                  userPath += '/Library/WorldOpenNetwork/keystore';
                }
                ...
                if (
                  ...
                  userPath += '/.worldopennetwork/keystore';
                }

                if (process.platform === 'win32') {
                  userPath = `${
                    Settings.appDataPath
                  }\\WorldOpenNetwork\\keystore`;
                }
              }
  ...
  if (process.platform === 'darwin' || process.platform === 'win32') {
    ...
    if (gethClient) {
      nodeSubmenu.push({
        label: `Gwon ${gethClient.version}`,
        checked: ethereumNode.isOwnNode && ethereumNode.isGeth,
        enabled: ethereumNode.isOwnNode,
        type: 'checkbox',
        click() {
          restartNode('gwon', null, 'fast', webviews);
        }
      });
    }
