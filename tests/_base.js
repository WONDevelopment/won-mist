...
exports.mocha = (_module, options) => {
  ...
  _module.exports[options.name || path.basename(_module.filename)] = {
    *before() {
      ...
      const appFileName = options.app === 'wallet' ? 'Won Wallet' : 'Won Mist';
      const platformArch = `${process.platform}-${process.arch}`;
      console.info(`${appFileName} :: ${platformArch}`);

      let appPath;
      const ipcProviderPath = path.join(this.geth.dataDir, 'gwon.ipc');
...
const Utils = {
  ...
  *getRealAccountBalances() {
    let accounts = this.web3.won.accounts;

    let balances = accounts.map(
      acc => `${this.web3.fromWei(this.web3.won.getBalance(acc), 'ether')}`
    );
