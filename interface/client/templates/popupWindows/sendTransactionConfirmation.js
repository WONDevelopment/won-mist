...
Template['popupWindows_sendTransactionConfirmation'].onCreated(function() {
  ...
  this.autorun(function() {
    ...
    if (data) {
      ...
      if (!data.gasPrice) {
        web3.won.getGasPrice(function(e, res) {
          if (!e) {
      ...
      if (data.to) {
        web3.won.getCode(data.to, function(e, res) {
      ...
      if (data.from) {
        web3.won.getCode(data.from, function(e, res) {
          if (!e && res && res.length > 2) {
...
Template['popupWindows_sendTransactionConfirmation'].helpers({
  ...
  /**
    Calculates the fee used for this transaction in won
  ...
  /**
    Calculates the provided gas amount in won
  ...
  providedGas: function() {
    ...
    if (gas && this.gasPrice) {
      const balance = web3.utils
        .toBN(gas || 0)
        .mul(web3.utils.toBN(this.gasPrice || 0));
      return EthTools.formatBalance(balance, '0,0.0[0000000]', 'won');
    }
  },
  /**
...
Template['popupWindows_sendTransactionConfirmation'].events({
  /**
    Gets the new provided gas in won amount and calculates the resulting providedGas
  ...
  'submit form': async function(e, template) {
    ...
    // get nonce
    const nonce = await web3.won.getTransactionCount(data.from);
    ...
    // sign transaction
    await web3.won.personal.signTransaction(tx, pw || '', function(
      error,
      result
    ) {
      pw = null;
    ...
    // send transaction!
    web3.won.sendSignedTransaction(signedTx, (error, hash) => {
      TemplateVar.set(template, 'unlocking', false);
      if (error) {
