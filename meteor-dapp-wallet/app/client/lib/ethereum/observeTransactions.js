...
addTransaction = function(log, from, to, value) {
  ...
  var block = web3.won.getBlock(log.blockNumber, false, function(err, block) {
    if (!err) {
      web3.won.getTransaction(log.transactionHash, function(err, transaction) {
        if (!err && transaction) {
          web3.won.getTransactionReceipt(log.transactionHash, function(
            err,
            receipt
          ) {
...
var updateTransaction = function(newDocument, transaction, receipt) {
  ...
  if (receipt && transaction) {
    ...
    if (!newDocument.contractAddress && receipt.contractAddress) {
      web3.won.getCode(receipt.contractAddress, function(e, code) {
        if (!e && code.length > 2) {
          Transactions.update(
            { _id: id },
            {
  ...
  if (newDocument.outOfGas) {
    ...
    if (EthAccounts.findOne({ address: newDocument.from })) {
      web3.won.getBalance(newDocument.from, newDocument.blockNumber, function(
        e,
        now
      ) {
        if (!e) {
...
observeTransactions = function() {
  ...
  var checkTransactionConfirmations = function(tx) {
    ...
    if (!tx.confirmed && tx.transactionHash) {
      var updateTransactions = function(e, blockHash) {
        ...
        if (!e) {
          ...
          if (
            ...
            web3.won.getTransaction(tx.transactionHash, function(
              e,
              transaction
            ) {
              web3.won.getTransactionReceipt(tx.transactionHash, function(
                e,
                receipt
              ) {
              ...
              web3.won.getTransactionReceipt(tx.transactionHash, function(
                ...
                if (!e) {
                  ...
                  if (!transaction || !transaction.blockNumber) {
                    ...
                    web3.won.getBlock(transaction.blockNumber, function(
                      e,
                      block
                    ) {
                      if (!e) {
      ...
      var subscription = web3.won.subscribe('newBlockHeaders', function(
        error,
        result
      ) {
