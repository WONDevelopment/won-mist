...
var getDataField = function() {
  ...
  if (selectedToken && selectedToken !== 'won') {
...
Template['views_send'].onCreated(function() {
  ...
  if (FlowRouter.getRouteName() === 'deployContract') {
    ...
    TemplateVar.set('selectedToken', 'won');

    // Send funds
  } else {
    TemplateVar.set('selectedAction', 'send-funds');
    TemplateVar.set('selectedToken', FlowRouter.getParam('token') || 'won');
  }
  ...
  template.autorun(function(c) {
    ...
    if (!c.firstRun && TemplateVar.get('selectedToken') === 'won') {
      TemplateVar.set(
        'amount',
        EthTools.toWei(
...
Template['views_send'].helpers({
  ...
  total: function(ether) {
    ...
    if (TemplateVar.get('selectedToken') === 'won') {
      amount =
...
Template['views_send'].events({
  ...
  'click .select-token input': function(e, template) {
    ...
    if (value === 'won') TemplateVar.setTo('.dapp-data-textarea', 'value', '');
  ...
  'submit form': function(e, template) {
    ...
    if (selectedAccount && !TemplateVar.get('sending')) {
      ...
      if (
        ...
        (!selectedAccount.owners || tokenAddress === 'won')
      )
        return GlobalNotification.warning({
